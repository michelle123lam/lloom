// Bubble Matrix component in d3
// Adapted from: https://github.com/analyzer2004/bubblematrix
...
export function render({model, el}) {
    let data_orig = () => model.get("data");
    let div = document.createElement("div");
    
    if (data_orig()) {
        const data = JSON.parse(data_orig());
        const width = 1200;
        const height = 800;
        const cur_color = "#82C1FB";
        const chart = new BubbleMatrix()
            .size([width, height])
            .options({
