...
class Session:
    ...
    def show_session_summary(self):
        ...
        print(f"Total time: {total_time:0.2f} sec")
    ...
    def __get_selection_widget(self, options_dict):
        ...
        def on_checkbox_change(change):
            c_names = [c.description for c in options_widget.children if c.value]
            selected_concepts = {c_id: c for c_id, c in self.concepts.items() if c.name in c_names}
    ...
    def vis(self, cols_to_show=[], custom_groups={}, show_highlights=True, norm_by="concept"):
        active_concepts = self.__get_active_concepts()
        active_score_dfs = [self.results[c_id] for c_id in active_concepts.keys()]
